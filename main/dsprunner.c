...
static esp_err_t is_valid_equalizer_samplerate(int samplerate) {
	if ((samplerate != 11025) && (samplerate != 22050) && (samplerate != 44100)
			&& (samplerate != 48000)) {
		ESP_LOGE(TAG,
				"The sample rate should be only 11025Hz, 22050Hz, 44100Hz, 48000Hz, here is %dHz. (line %d)",
				samplerate, __LINE__);
...
static esp_err_t is_valid_equalizer_channel(int channel) {
	if ((channel != 1) && (channel != 2)) {
		ESP_LOGE(TAG,
				"The number of channels should be only 1 or 2, here is %d. (line %d)",
				channel, __LINE__);
		return ESP_ERR_INVALID_ARG;
	}
	return ESP_OK;
...
esp_err_t equalizer_set_info(audio_element_handle_t self, int rate, int ch) {
	...
	if ((is_valid_equalizer_samplerate(rate) != ESP_OK)
		...
		ESP_LOGI(TAG,
				"The reset sample rate and channel of audio stream are %d %d.",
				rate, ch);
...
esp_err_t equalizer_set_gain_info(audio_element_handle_t self, int index,
	...
	equalizer_t *equalizer = (equalizer_t*) audio_element_getdata(self);
	if ((equalizer->channel == 2) && (is_channels_gain_equal == true)) {
		if ((index < 0) || (index > NUMBER_BAND)) {
			ESP_LOGE(TAG,
					"The range of index for audio gain of equalizer should be [0 9]. Here is %d. (line %d)",
					index, __LINE__);
			return ESP_ERR_INVALID_ARG;
		}
		...
		ESP_LOGI(TAG, "The reset gain[%d] and gain[%d] of audio stream are %d.",
				index, pos_index_channel, value_gain);
		equalizer->gain_flag = 1;
		equalizer->set_gain[index] = value_gain;
		equalizer->set_gain[pos_index_channel] = value_gain;
		return ESP_OK;
	} else {
		if ((index < 0) || (index > NUMBER_BAND * 2)) {
			ESP_LOGE(TAG,
					"The number of channels is %d. The range of index for audio gain of equalizer should be [0 %d]. Here is %d. (line %d)",
					equalizer->channel, (equalizer->channel * NUMBER_BAND - 1),
					index, __LINE__);
			return ESP_ERR_INVALID_ARG;
		}
		if (equalizer->set_gain[index] == value_gain) {
		...
		ESP_LOGI(TAG, "The reset gain[%d] of audio stream is %d.", index,
				value_gain);
		equalizer->gain_flag = 1;
		equalizer->set_gain[index] = value_gain;
		return ESP_OK;
...
static esp_err_t equalizer_open(audio_element_handle_t self) {
	...
	if (equalizer->num_band != 10 && equalizer->num_band != 15
			&& equalizer->num_band != 25 && equalizer->num_band != 31) {
		ESP_LOGE(TAG,
				"The number of bands should be one of 10, 15, 25, 31, here is %d. (line %d)",
				equalizer->num_band, __LINE__);
		return ESP_ERR_INVALID_ARG;
	}
	...
	if (equalizer->use_xmms_original_freqs != 0
		...
		ESP_LOGE(TAG,
				"The use_xmms_original_freqs should be only 0 or 1, here is %d. (line %d)",
				equalizer->use_xmms_original_freqs, __LINE__);
		return ESP_ERR_INVALID_ARG;
	}
	...
	equalizer->eq_handle = esp_equalizer_init(equalizer->channel,
			equalizer->samplerate, equalizer->num_band,
			equalizer->use_xmms_original_freqs);
	if (equalizer->eq_handle == NULL) {
		ESP_LOGE(TAG, "failed to do equalizer initialization. (line %d)",
				__LINE__);
		return ESP_FAIL;
	}
	for (int i = 0; i < equalizer->channel; i++) {
		for (int j = 0; j < NUMBER_BAND; j++) {
			esp_equalizer_set_band_value(equalizer->eq_handle,
					equalizer->set_gain[NUMBER_BAND * i + j], j, i);
		}
	}
...
static int equalizer_process(audio_element_handle_t self, char *in_buffer,
		int in_len) {
	equalizer_t *equalizer = (equalizer_t*) audio_element_getdata(self);
	int ret = 0;
	...
	if (r_size > 0) {
		...
		ret = esp_equalizer_process(equalizer->eq_handle,
				(unsigned char*) equalizer->buf, r_size, equalizer->samplerate,
				equalizer->channel);
